...
import { Download, Edit3, Sparkles, Building, FileText, AlertCircle, ArrowLeft } from 'lucide-react';
...
interface OptimizedCoverLetterGeneratorProps {
  resumeData: ResumeData;
  onBack: () => void;
}

const OptimizedCoverLetterGenerator: React.FC<OptimizedCoverLetterGeneratorProps> = memo(({ 
  resumeData, 
  onBack 
}) => {
  ...
  useEffect(() => {
    const loadSavedData = async () => {
      try {
        const savedApp = await loadApplicationFromFirebase();
        if (savedApp) {
          setApplication(savedApp);
          setJobDescription(savedApp.jobDescription || '');
        }
      } catch (error) {
        console.warn('Could not load saved application:', error);
      }
    };
    loadSavedData();
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-800 mb-2">Cover Letter Generator</h1>
...
OptimizedCoverLetterGenerator.displayName = 'OptimizedCoverLetterGenerator';

export default OptimizedCoverLetterGenerator;